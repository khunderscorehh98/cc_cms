    ...
    <div>
        <v-container>
            ...
            <v-data-table :headers="headers" :items="deletedUsers" class="elevation-1">
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title>Archived Applicants</v-toolbar-title>
                        <v-spacer></v-spacer>
                    </v-toolbar>
                </template>
    ...
    methods: {
        fetchAllUsers() {
            this.$axios
                .get("/users")
                .then((response) => {
                    this.deletedUsers = response.data.filter((user) => user.Archive === 1);
                })
                .catch((error) => {
        ...
        restoreUser(user) {
            this.$axios
                .put(`/users/${user.user_id}`, { Archive: 0 }) // Restore user
                .then(() => {
                    // Update local state
